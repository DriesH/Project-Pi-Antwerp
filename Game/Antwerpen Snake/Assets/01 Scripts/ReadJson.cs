...
public class ReadJson : MonoBehaviour {
  ...
  {
    if (!urlApi.StartsWith("<!DOCTYPE html>")) //if the database couldn't load
    {
  ...
  void GetQuestionsFromDatabase(string urlApi)
  {
    ...
    for (int i = 0; i < itemData["appvragen"].Count; i++) //for each project
    {
      databaseQuestions.Add((string)itemData["appvragen"][i]["question"]); 
      databaseIDQuestions.Add((string)itemData["appvragen"][i]["idAppquestions"]);
      databaseIDProjectQuestions.Add((string)itemData["appvragen"][i]["idProject"]);
      numberOfQuestions++;
    }
    readyToPlay = true;
  }
